...
class HybridCaptureAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    if plannedAction in bestActions:
      action = plannedAction
    else:
      nonStopBest = [candidate for candidate in bestActions if candidate != Directions.STOP]
      action = sorted(nonStopBest or bestActions)[0]
  ...
  def trackFoodDefenseEvents(self, gameState):
    ...
    for pos in missingFood:
      self.shared['recent_events'].append({'pos': pos, 'ttl': 5, 'kind': 'food'})
    for pos in missingCapsules:
      self.shared['recent_events'].append({'pos': pos, 'ttl': 10, 'kind': 'capsule'})
  ...
  def mustRetreatAgent(self, gameState, agentIndex):
    ...
    if carrying >= threshold:
      return True
    if carrying >= 5 and scaredWindow < 8:
      return True
    if carrying >= 3 and homeDist >= 7 and scaredWindow < 5:
      return True
  ...
  def pickInterceptTarget(self, gameState):
    ...
    for enemy in invaders:
      ...
      if pos is not None:
        ...
        myToExit = self.getMazeDistance(myPos, exitCell)
        enemyToExit = self.getMazeDistance(pos, exitCell)
        # Only cut off the exit when we can realistically beat or match the invader there.
        # If we are too far away, chasing the invader's current position is usually better.
        if enemyToExit <= myToExit:
          candidateTargets.append((exitCell, 'urgent_exit'))
        else:
  ...
  def pickPatrolTarget(self, gameState):
    ...
    preferredUpper = self.preferredUpperLane(gameState)
  ...
  def pickAttackTarget(self, gameState):
    ...
    clusterSizes = self.computeFoodClusterSizes(candidateFoods)
    ...
    for pellet in candidateFoods:
      ...
      if scaredWindow < 4 and threatPos is not None:
        enemyToFood = self.getMazeDistance(threatPos, pellet)
        if depth >= 2 and enemyToFood <= distance + 1:
          continue

      clusterBonus = min(4.0, 0.8 * clusterSizes.get(pellet, 1))
  ...
  def scoreActions(self, gameState, legalActions, mode, target, plan):
    ...
    for action in legalActions:
      ...
      if mode == 'attack':
        ...
        if self.inOpeningPhase(gameState) and self.matchesPreferredLane(nextPos, self.preferredUpperLane(gameState)):
          score += 1.5
      elif mode == 'retreat':
  ...
  def applyTacticalSearch(self, gameState, actionScores, mode, target):
    ...
    for action, baseScore in bestCandidates:
      ...
      adjustedBase = baseScore
      for otherEnemy in visibleEnemies:
        otherState = gameState.getAgentState(otherEnemy)
        otherPos = gameState.getAgentPosition(otherEnemy)
        if otherPos is None or otherState.isPacman or otherState.scaredTimer > 2:
          continue
        distance = self.getMazeDistance(nextPos, otherPos)
        if distance <= 1:
          adjustedBase -= 40.0
        elif distance == 2:
          adjustedBase -= 15.0
      ...
      if not enemyActions:
        rescored[action] = round(adjustedBase, 6)
        continue
      ...
      rescored[action] = round(0.7 * adjustedBase + 0.3 * min(replyValues), 6)
    return rescored
  ...
  def preferredUpperLane(self, gameState):
    starts = self.shared.get('agent_starts', {})
    teamStarts = [(agent, pos) for agent, pos in starts.items() if agent in self.getTeam(gameState)]
    if len(teamStarts) < 2:
      return self.slot == 0
